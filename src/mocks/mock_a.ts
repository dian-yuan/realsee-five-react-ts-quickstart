import { Tag } from '@realsee/dnalogel'
import { ContentType } from '@realsee/dnalogel/dist'
import { Mode } from '@realsee/five'
import { Vector3 } from 'three'
export const TagsList: Tag[] = [
  // {
  //   dimensionType: '2D',
  //   pointType: 'PointTag',
  //   contentType: 'ImageText',
  //   enabled: true,
  //   position: [-1.3311771624715265, 0, -0.8947385152744657],
  //   data: {
  //     text: '2D图文标签哈哈哈哈哈',
  //     mediaData: [
  //       {
  //         url: 'http://vr-static.realsee-cdn.cn/release/web/arrow.c5d028e4.png',
  //         type: 'Image',
  //       },
  //       {
  //         url: 'http://vr-static.realsee-cdn.cn/release/web/arrow.c5d028e4.png',
  //         type: 'Image',
  //       },
  //       {
  //         url: 'http://vr-static.realsee-cdn.cn/release/web/arrow.c5d028e4.png',
  //         type: 'Image',
  //       },
  //     ],
  //   },
  //   id: 1,
  //   fiveState: {
  //     mode: 'Panorama' as Mode,
  //     panoIndex: 0,
  //   },
  // },
  // {
  //   dimensionType: '3D',
  //   pointType: 'PlaneTag',
  //   contentType: 'MediaPlane',
  //   // stickType:'3DPoint',
  //   enabled: true,
  //   position: [
  //     [-0.27, 0.1172, 0.028],
  //     [0.27, 0.1172, 0.028],
  //     [0.27, 1.6172, 0.028],
  //     [-0.27, 1.6172, 0.028],
  //   ],
  //   data: {
  //     mediaData: [
  //       {
  //         url: 'http://videos.pexels.com/video-files/3015527/3015527-sd_640_360_24fps.mp4',
  //         videoCoverUrl: 'http://vr-static.realsee-cdn.cn/release/web/jsl/test.65720b34.png',
  //         type: 'Video',
  //       },
  //       {
  //         url: 'http://videos.pexels.com/video-files/3015527/3015527-sd_640_360_24fps.mp4',
  //         videoCoverUrl: 'http://vr-static.realsee-cdn.cn/release/web/jsl/test.65720b34.png',
  //         type: 'Video',
  //       },
  //     ],
  //   },
  //   fiveState: {
  //     mode: 'Panorama' as Mode,
  //     panoIndex: 0,
  //     // longitude: 2.706451797307377,
  //     // latitude: 0.05631416156432678,
  //     // fov: 95,
  //     // offset: {
  //     //   x: -0.618914008140564,
  //     //   y: 1.1130503593145502,
  //     //   z: -1.6621500253677368,
  //     // } as Vector3,
  //     // distance: 0,
  //   },
  // },
  // {
  //   // dimensionType: '3D',
  //   pointType: 'PlaneTag',
  //   contentType: 'MediaPlane',
  //   // stickType:'Plane',
  //   enabled: true,
  //   position: [
  //     [-4.344637059045372, 0.8661640292431658, -2.339700937271118 + 0.01],
  //     [-1.4781933464383068, 0.8696103907729694, -2.339700937271118 + 0.01],
  //     [-1.4865722199099451, 2.1293509755511106, -2.3397009372711177 + 0.01],
  //     [-4.343575500347607, 2.1145151110093137, -2.3397009372711177 + 0.01],
  //   ],
  //   fiveState: {
  //     mode: 'Panorama' as Mode,
  //     panoIndex: 0,
  //   },
  //   data: {
  //     mediaData: [
  //       {
  //         // url: 'http://10.33.192.249:81/local/uwork/vrcustomer/XINGZHEN/jXrqvOgEU8l4ZH4m7O/2f3031fadbbaab2ea7a631df854be567.jpg',
  //         // url: 'http://vr-static.realsee-cdn.cn/release/web/arrow.c5d028e4.png',
  //         // url: 'http://test-vr-public.realsee-cdn.cn/test/uwork/vrcustomer/REALSEE_TECH/gmYwV9YON6Hz0avsvo/2022-11-15_18-32-46.jpg',
  //         url: 'http://vr-static.realsee-cdn.cn/release/web/jsl/test.65720b34.png',
  //         // url: 'http://vr-public.realsee-cdn.cn/release/uwork/vrcustomer/REALSEE_TECH/Vxn65erkq7cBbNWtVx/2023-03-14_15-55-31.jpeg?imageMogr2/thumbnail/750x%3E/quality/70',
  //         type: 'Image',
  //       },
  //     ],
  //   }
  // },
  // {
  //   dimensionType: '3D',
  //   pointType: 'PlaneTag',
  //   contentType: 'MediaPlane',
  //   enabled: true,
  //   fiveState: {
  //     mode: 'Panorama' as Mode,
  //     panoIndex: 0,
  //   },
  //   position: [
  //     [-0.16450234208682218, 1.2079436283327727, -2.3397009372711186],
  //     [0.4534614669340434, 1.2009985563997523, -2.3397009372711186],
  //     [0.4588598077301688, 1.9071464262166917, -2.339700937271118],
  //     [-0.16094619454331782, 1.9098602633949615, -2.3397009372711186],
  //   ],
  //   data: {
  //     mediaData: [
  //       {
  //         url: 'http://vr-static.realsee-cdn.cn/release/web/catCoding.1cd4e989.gif',
  //         // url: 'http://vr-static.realsee-cdn.cn/release/web/jsl/test.65720b34.png',
  //         type: 'Image',
  //       },
  //       {
  //         url: 'http://vr-static.realsee-cdn.cn/release/web/catCoding.1cd4e989.gif',
  //         type: 'Image',
  //       },
  //     ],
  //   },
  // },
  // {
  //   dimensionType: '3D',
  //   pointType: 'PlaneTag',
  //   contentType: 'MediaPlane',
  //   enabled: true,
  //   position: [
  //     [-1.9866843325110417, 2.220446049250313e-16, 0.37528638936879805 + 0.01],
  //     [-0.9866843325110417, 2.220446049250313e-16, 0.37528638936879805 + 0.01],
  //     [-1.9866843325110417, 1.220446049250313e-16, 0.37528638936879805 + 0.01],
  //     [-0.9866843325110417, 1.220446049250313e-16, 0.37528638936879805 + 0.01],
  //   ],
  //   data: {
  //     mediaData: [
  //       {
  //         // url: '//vr-static.realsee-cdn.cn/release/web/arrow.c5d028e4.png',
  //         url: 'http://vr-static.realsee-cdn.cn/release/web/jsl/test.65720b34.png',
  //         type: 'Image',
  //       },
  //     ],
  //   },
  //   fiveState: {
  //     mode: 'Panorama' as Mode,
  //     panoIndex: 3,
  //     longitude: 2.706451797307377,
  //     latitude: 0.05631416156432678,
  //     fov: 95,
  //     offset: {
  //       x: -0.618914008140564,
  //       y: 1.1130503593145502,
  //       z: -1.6621500253677368,
  //     } as Vector3,
  //     distance: 0,
  //   },
  // },
  // {
  //   dimensionType: '2D',
  //   pointType: 'PointTag',
  //   contentType: 'Marketing',
  //   enabled: true,
  //   position: [-2.6990631227056436, 0.018038999289274438, -0.3673991100072416],
  //   data: {
  //     title: '2D营销标签',
  //     brandTags: ['品牌名称'],
  //     tags: ['标签描述', '标签描述'],
  //     headerPictureUrl: 'http://vr-static.realsee-cdn.cn/release/web/cardHeader.19ff7aa0.png',
  //     highlightText: '立即查看',
  //     price: {
  //       value: '20.30',
  //       unit: '元起',
  //     },
  //   },
  //   style: {
  //     point: {
  //       url: 'http://vr-static.realsee-cdn.cn/release/web/carTagDotContent.08ace25e.png',
  //       width: 20
  //     },
  //   },
  //   id: 3,
  //   fiveState: {
  //     mode: 'Panorama' as Mode,
  //     panoIndex: 0,
  //     longitude: 1.4711282761387834,
  //     latitude: 0.020969743131104737,
  //     fov: 95,
  //     offset: {
  //       x: 0.09141919761896133,
  //       y: 1.1138173893692536,
  //       z: -0.08654399961233139,
  //     } as Vector3,
  //     distance: 0,
  //   },
  // },
  {
    dimensionType: '3D',
    pointType: 'PointTag',
    contentType: 'Text',
    stickType:'3DPoint',
    enabled: true,
    normal: [-2.6990631227056436, 0.018038999289274438, -0.3673991100072416],
    position: [0.927103332208612, 0.8445514464586326, -0.6591370084540737],
    data: {
      text: '3D文本标签哈哈',
      title: '3D文本标签哈哈',
    },
    id: 4,
    fiveState: {
      mode: 'Panorama' as Mode,
      panoIndex: 0,
      longitude: 5.493428678855368,
      latitude: -0.10500571410520228,
      fov: 95,
      offset: {
        x: 0.09141919761896133,
        y: 1.1138173893692536,
        z: -0.08654399961233139,
      } as Vector3,
      distance: 0,
    },
  },
  // {
  //   dimensionType: '2D',
  //   pointType: 'PointTag',
  //   contentType: 'Audio',
  //   // "position": [
  //   //     0.17501044773948682,
  //   //     1.1010380993204227,
  //   //     -2.339700937271118
  //   // ],
  //   position: [-2.1892900647187052, 1.556751598267469, 1.0212990045547485],
  //   data: {
  //     appearance: 'plane',
  //     audioUrl: 'http://vr-static.realsee-cdn.cn/release/web/test.145da774.m4a',
  //     text: 'test.m4a',
  //   },
  //   id: 51,
  //   fiveState: {
  //     mode: 'Panorama' as Mode,
  //     panoIndex: 0,
  //     longitude: 6.151207373723298,
  //     latitude: -0.24015893081017836,
  //     fov: 95,
  //     offset: {
  //       x: 0.09141919761896133,
  //       y: 1.1138173893692536,
  //       z: -0.08654399961233139,
  //     } as Vector3,
  //     distance: 0,
  //   },
  // },
  // {
  //   dimensionType: '2D',
  //   pointType: 'PointTag',
  //   contentType: 'Audio',
  //   // "position": [
  //   //     0.17501044773948682,
  //   //     1.1010380993204227,
  //   //     -2.339700937271118
  //   // ],
  //   position: [-2.1892900647187052, 1.6567515982674699, 1.0212990045547485],
  //   data: {
  //     appearance: 'plane',
  //     audioUrl: 'http://vr-static.realsee-cdn.cn/release/web/Bling.678be963.mp3',
  //     text: '2D音频标签标题标题标题标标题标题标题标题标题标题标题标题标题标题标题标标题标题',
  //   },
  //   id: 5,
  //   fiveState: {
  //     mode: 'Panorama' as Mode,
  //     panoIndex: 0,
  //     longitude: 6.151207373723298,
  //     latitude: -0.24015893081017836,
  //     fov: 95,
  //     offset: {
  //       x: 0.09141919761896133,
  //       y: 1.1138173893692536,
  //       z: -0.08654399961233139,
  //     } as Vector3,
  //     distance: 0,
  //   },
  // },
  // {
  //   dimensionType: '2D',
  //   pointType: 'PointTag',
  //   contentType: 'Link',
  //   enabled: true,
  //   position: [-0.8070892936181384, 1.1621840046490304, 1.0212990045547485],
  //   data: {
  //     linkType: 'vr',
  //     text: '2D跳转标签',
  //   },
  //   fiveState: {
  //     mode: 'Panorama' as Mode,
  //     panoIndex: 0,
  //   },
  //   id: 6,
  // },
  // {
  //   dimensionType: '2D',
  //   pointType: 'PointTag',
  //   contentType: 'Panorama',
  //   enabled: true,
  //   position: [-0.8070892936181384, 0.1, 1.0212990045547485],
  //   fiveState: {
  //     mode: 'Panorama' as Mode,
  //     panoIndex: 0,
  //   },
  //   data: {
  //     mediaData: [
  //       {
  //         type: 'Image',
  //         url: 'https://test-vr-public.realsee-cdn.cn/test/uwork/vrcustomer/REALSEE_YUNYING/wmJkr0bkJ6FdngbtXx/2023-02-23_15-22-16.png',
  //         thumbnail:
  //           'https://test-vr-public.realsee-cdn.cn/test/uwork/vrcustomer/REALSEE_YUNYING/wmJkr0bkJ6FdngbtXx/2023-02-23_15-22-16.png',
  //       },
  //     ],
  //   },
  //   id: 100,
  // },
  {
    id: 'MediaModel',
    stickType: 'Model',
    contentType: 'MediaModel',
    fiveState: {
      distance: 0,
      fov: 10,
      latitude: 0,
      longitude: 1.5817071602126802,
      mode: 'Panorama' as Mode,
      offset: { x: 2.60088, y: 0.0440728, z: -3.06787 } as Vector3,
      panoIndex: 1,
    },
    position: [-1.4006045858954423, 0.005288434039257162, -1.654760281604037],
    data: {
      appearance: 'plane',
      title: null,
      description: null,
      mediaData: [
        {
          type: 'Video',
          url: 'http://videos.pexels.com/video-files/3015527/3015527-sd_640_360_24fps.mp4',
          videoCoverUrl:
            'https://vrlab-public.ljcdn.com/common/file/web/430987ac-4518-49ca-ae9a-3477526cd909.png',
        },
      ],
      targetUrl: null,
      modelType: 'billboard1',
      type: 'billboard1',
      objectFit: 'contain',
      // action: { type: 'window', url: 'http://videos.pexels.com/video-files/3015527/3015527-sd_640_360_24fps.mp4' },
      modelUrl: 'https://vrlab-public.ljcdn.com/common/file/web/1787333b-7ee1-44ce-ba48-ed7d43b8d406.gltf',
      // defaultImageUrl: 'https://vrlab-public.ljcdn.com/common/file/web/262e33a5-a975-45ad-9821-1f658bd0e87a.png',
      // previewUrl: 'https://vrlab-public.ljcdn.com/common/file/web/430987ac-4518-49ca-ae9a-3477526cd909.png',
      // iconUrl: 'https://vrlab-public.ljcdn.com/common/file/web/ef5ee840-eb42-416c-9ce6-15e25a643e6e.png',
      // size: '540x1500',
      mediaPosition: [
        [-0.27, 0.1172, 0.028],
        [0.27, 0.1172, 0.028],
        [0.27, 1.6172, 0.028],
        [-0.27, 1.6172, 0.028],
      ],
    },
    // style: { point: { iconType: 'circle' } },
    config: { 
      visibleConfig: { visiblePanoIndex: 'all' }, 
      // unfoldedConfig: { autoUnfold: { enable: true } } 
    },
    matrix: [
      0.3012921352579293, 0, 0.9535318816021398, 0, 0, 1, 0, 0, -0.9535318816021398, 0, 0.3012921352579293, 0, 1.347059457861614,
      0.005288434039257162, -2.056130746849461, 1,
    ],
  },
  {
    id: 'MediaModel',
    stickType: 'Model',
    contentType: 'MediaModel',
    fiveState: {
      distance: 0,
      fov: 100,
      latitude: 0,
      longitude: 1.5817071602126802,
      mode: 'Panorama' as Mode,
      offset: { x: 2.60088, y: 0.0440728, z: -3.06787 } as Vector3,
      panoIndex: 1,
    },
    position: [-1.4006045858954423, 0.005288434039257162, -1.654760281604037],
    data: {
      appearance: 'plane',
      title: null,
      description: null,
      mediaData: [
        {
          type: 'Image',
          url: 'https://test-vr-public.realsee-cdn.cn/test/uwork/vrcustomer/REALSEE_YUNYING/wmJkr0bkJ6FdngbtXx/2023-02-23_15-22-16.png',
        },
        {
          type: 'Image',
          url: 'https://test-vr-public.realsee-cdn.cn/test/uwork/vrcustomer/REALSEE_YUNYING/wmJkr0bkJ6FdngbtXx/2023-02-23_15-22-16.png',
        },
      ],
      targetUrl: null,
      modelType: 'billboard1',
      objectFit: 'contain',
      action: { type: 'iframe', url: '' },
      type: 'billboard1',
      modelUrl: 'https://vrlab-public.ljcdn.com/common/file/web/1787333b-7ee1-44ce-ba48-ed7d43b8d406.gltf',
      defaultImageUrl: 'https://vrlab-public.ljcdn.com/common/file/web/262e33a5-a975-45ad-9821-1f658bd0e87a.png',
      previewUrl: 'https://vrlab-public.ljcdn.com/common/file/web/430987ac-4518-49ca-ae9a-3477526cd909.png',
      iconUrl: 'https://vrlab-public.ljcdn.com/common/file/web/ef5ee840-eb42-416c-9ce6-15e25a643e6e.png',
      size: '540x1500',
      mediaPosition: [
        [-0.27, 0.1172, 0.028],
        [0.27, 0.1172, 0.028],
        [0.27, 1.6172, 0.028],
        [-0.27, 1.6172, 0.028],
      ],
    },
    // style: { point: {width:100} },
    config: { visibleConfig: { visiblePanoIndex: 'all' }, unfoldedConfig: { autoUnfold: { enable: false } } },
    matrix: [
      -0.5596409874088015, 0, 0.828735159874432, 0, 0, 1, 0, 0, -0.828735159874432, 0, -0.5596409874088015, 0, 1.1166287008785034,
      0.005288434039257162, 0.5336379042536049, 1,
    ],
  },
  // {
  //   pointType: 'PointTag',
  //   stickType:'2DPoint',
  //   contentType: 'Text',
  //   enabled: true,
  //   position: [-2.0543282763294135, 0.3319934118281651, -2.2450259661695933],
  //   data: {
  //     appearance: 'plane',
  //     text: '这 是 一 个 文本标签🙋‍♂️🙋‍♂️🙋‍♂️🙋‍♂️🙋‍♂️🙋‍♂️🙋‍♂️，它有很长的标题标题标题标题标题标题标题标题标题标题标题标题标题',
  //     description:
  //       '收到会尽快垃圾袋索拉卡鲸打卡拉萨金卡到拉萨金卡老大洒基卡拉多撒金卡溜达鸡撒溜达鸡算啦收到会尽快垃圾袋索拉卡鲸打卡拉萨金卡到拉萨金卡老大洒基卡拉多撒金卡溜达鸡撒溜达鸡算啦收到会尽快垃圾袋索拉卡鲸打卡拉萨金卡到拉萨金卡老大洒基卡拉多撒金卡溜达鸡撒溜达鸡算啦收到会尽快垃圾袋索拉卡鲸打卡拉萨金卡到拉萨金卡老大洒基卡拉多撒金卡溜达鸡撒溜达鸡算啦收到会尽快垃圾袋索拉卡鲸打卡拉萨金卡到拉萨金卡老大洒基卡拉多撒金卡溜达鸡撒溜达鸡算啦',
  //   },
  //   style: {
  //     point: {
  //       url: 'http://vr-static.realsee-cdn.cn/release/web/carTagDotContent.08ace25e.png',
  //     },
  //   },
  //   id: 1,
  //   fiveState: {
  //     mode: 'Panorama' as Mode,
  //     panoIndex: 0,
  //     longitude: 0.6087052772120609,
  //     latitude: -0.0392156862745098,
  //     fov: 95,
  //     offset: {
  //       x: 0.09141919761896133,
  //       y: 1.1138173893692536,
  //       z: -0.08654399961233139,
  //     } as Vector3,
  //     distance: 0,
  //   },
  // },
]